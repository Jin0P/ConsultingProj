...
pred_prob <- predict(lasso.mod[[i]], s = bestlam, newx = x_test,,type="response")
...
pred_prob <- predict(lasso.mod[[i]], s = bestlam, newx = x_test,,type="response")
predict_C_MUA <- ifelse(pred_prob > sum(y_train)/length(y_train),1,0)
spec.over[i] <- sum(predict_C_MUA*y_test)/sum(y_test)
}

sum(predict_C_MUA); sum(y_test)
predict(lasso.mod[[1]],s=bestlam[1],type="coefficients")
predict(lasso.mod[[5]],s=bestlam[5],type="coefficients")
...
```{r}
# reference : https://statisticsglobe.com/stratified-k-fold-cross-validation-r
...
for (y_i in 0:1 ) {                                          # Create stratified folds
...
for(i in 1:5){
y_train = as.vector(data_ex[fold!=i,1]$y)
x_train = as.matrix(data_ex[fold!=i,-c(1,47)])


y_test = as.vector(data_ex[fold==i,1]$y)
x_test = as.matrix(data_ex[fold==i,-c(1,47)])
...
lasso.mod.st[[i]] = glmnet(x_train, y_train, alpha= 1, lambda = bestlam,family="binomial")
pred_prob <- predict(lasso.mod[[i]], s = bestlam, newx = x_test,type="response")
predict_C_MUA <- ifelse(pred_prob > sum(y_train)/length(y_train),1,0)
spec.st[i] <- sum(predict_C_MUA*y_test)/ sum(y_test)
}
