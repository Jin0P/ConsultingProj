...
ggplot(data = MUA_data1, aes(x = date_diff, y=C_MUA)) +
  ...
  coord_flip()+
  labs(x = "# days between 2 TKAs",y = NULL, fill=NULL)+ theme(legend.position="none")
```
...
MUA_data1 %>% group_by(sex) %>%  count(Group2) %>% 
...
library(glmnet)
x1matrix = data.matrix(MUA_data1[,c(82:103)]) # ASA, disease, readmission when C_TKA
