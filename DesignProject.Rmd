...
We randomly divided the 16 roses into two blocks equally to make it balanced, and each block has 8 observations for $2^4$ factorial analysis so it is incomplete design.
...
Let’s look into the interaction term of ABD and AB. The plot # is the interaction plot of ABD. When D = 1, the slope of red line and the dashed blue line are not paralleled but quite similar. The sign of the slope are all negative and the two lines are located close to each other. But D = -1, for the roses that we didn’t store in the refrigerator at night, the two slope showed the opposite direction. We want the smallest y, assuming that mean the healthiest rose. In the interaction plot ABD, we can find that y has the smallest value when D = -1, B= -1 but only A = 1. This means that when not storing the roses in the refrigerator at night and putting tap water but putting the recommended amount of flower food, we can the roses stay healthy longer.  and the interaction plot of AB also shows that when B=-1 and A=1, we got the best result.  

The interaction plot of AB shows the relationship between flower food and water on the longevity of the roses. The two lines are not paralleled so the interaction effect is confirmed. In the interaction plot AB, we can find that y has the smallest value when B = -1 and A = 1. This means that when putting the recommended amount of flower food in tap water, the roses stay healthy longer. Interesting finding is when B=1 and A=1, so when putting the roses in filtered water adding the recommended amount of flower food, it is likely to get the worst result. 

##### e. Regression model 

We checked the linear regression result. Interestingly, the main effects A and B which are insignificant in ANOVA are significant in the regression model. 
...
###### (i) Normality assumption check
  To test the assumption of normal distribution, Shapiro-Wilk's W test and Q-Q plot are used. In Q-Q plot(plot #), the plotted residuals are not departed from the straight line, which means it is normally distributed. Shapiro-Wilk's test result is consistent with the finding in the QQ plot. The p-value of Shapiro test shows insignificant (p-value >0.05) so we can confirm that the assumption of normality meet.
  
###### (ii) Homogeneity of variance check 
 To test the assumption of homogeneity of variance, Levene's test and the scatter plot of residuals versus fitted value are used. The scatter plot of residuals versus fitted value is used to detect unequal error variances. The scatter plot (plot #) shows that a megaphone pattern so the assumption seems to be violated. The significance (p-value < 0.05) of the Levene's test confirmed the violation of the homogeneity of variance assumption. 
 
 ANOVA is robust to violations to the assumption of homogeneity of variance. So we basically can trust the findings. But also,the possibility that the conclusion of the analysis is invalid still remains. We can consider nonparametric alternative ANOVA test which do not assume equality of variance, such as Kruskal Wallis test. 
...
The null hypothesis is all effect of treatments are the same so it is written as $H_0:\tau_1 = \tau_2 = \tau_3 = \tau_4$. The alternative hypothesis is at least one of the effect of treatment is different. 
...
###### (ii) Homogeneity of variance check 
  To test the assumption of homogeneity of variance, Barlett's test and the scatter plot of residuals versus fitted value are used. The scatter plot (plot #) does not appear any pattern so the assumption of homogeneity of variance seems to be valid. The insignificance (p-value > 0.05) of the Barlett's test confirmed that the assumption of homogeneity of variance meets. 

##### d. Tukey comparisons
Since the insignificance of the cutting angle is confirmed, the Tukey comparison is not required as the ad hoc testing. However, since the box-plot shows some noticeable difference between cutting angle 45 (level1) and no cut (level4), we decided to check the Tukey comparisons. 
When level 4 is compared with other levels, the differences between two levels seems to be larger than the differences between other levels, but all of the confidence levels cover zero, so it shows there is no significant difference at the 5% level. Also, even at the 10% level, Tukey comparison shows that most of confidence levels cover zero except the comparison between level 1 and level 4. So, we could confirm that the insignificance of the cutting angle levels.  
