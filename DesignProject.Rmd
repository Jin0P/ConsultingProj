...
where y : the height difference between the first day and 12th day
    ...
    $\beta_j$ :  effect of jth blocks
    i : the number of level 1,2,3,4
    j : the number of block 1,2 

The assumption of RBCD is     
>  $ε_{ij} ~ N(0,\sigma^2)$

In RCBD, the residuals are assumed to be normally distributed with mean 0 and common variance $\sigma^2$.
    
###### (ii) Analysis

The null hypothesis is all effect of treatments are the same so it can be written as $H_0:\tau_1 = \tau_2 = \tau_3 = \tau_4$. The alternative hypothesis is at least one of the effect of treatment is different. 
